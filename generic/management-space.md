...
Deployment_Node(cloudgov, "cloud.gov", "Cloud Foundry PaaS") {
    ...
    System_Ext(log_service, "Log store", "Stores, monitors, and alerts on logs")
...
'Log shipping
Rel(management_drain, log_service, "ship logs", "stdout/stderr")
Rel(ms_drain, log_service, "ship logs", "stdout/stderr")
@enduml
```
